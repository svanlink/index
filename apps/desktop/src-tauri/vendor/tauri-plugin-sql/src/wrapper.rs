    ...
    pub(crate) async fn connect<R: Runtime>(
            ...
            .0
        {
            ...
            "sqlite" => {
                ...
                // LOCAL PATCH (Drive Project Catalog):
                // - Pin SQLite pool to 1 connection. Multi-connection pools break
                //   `BEGIN IMMEDIATE` / statement / COMMIT sequences when subsequent
                //   statements land on a different pooled connection than the BEGIN,
                //   surfacing as SQLITE_BUSY (code 5) "database is locked".
                // - Enable WAL journal mode for durable concurrent reads.
                // - Set a 10s busy_timeout so SQLite auto-retries transient BUSY.
